...
The timer password defaults to `lawnmower`. Set it via environment variable before building:
...
`docker-compose.yml` reads `${TIMER_PASSWORD:-lawnmower}` — if the variable isn't set it falls back to the default.
