...
from config import LAWN_CONSTELLATION_IDS, REGION_ID, NEIGHBOR_SYSTEM_NAMES
import esi_client


class SystemState:
    def __init__(self):
        self.constellation_data = {}     # cid -> {name, region_id, systems, is_lawn}
        self.neighbor_systems = {}       # sys_id -> {name, system_id, security_status, region_name}
        self.lawn_constellation_ids_set = set()
        self.lawn_system_ids = set()
        self.all_monitored_ids = set()   # All ~87 system IDs (TKE + neighbors)
...
def resolve_all_systems(s: SystemState):
    """Load all TKE constellations + neighbor systems from ESI, populating s.*."""
    import time as _time

    t_start = _time.monotonic()
    s.lawn_constellation_ids_set = set(LAWN_CONSTELLATION_IDS)
    ...
    for cid, info in constellation_infos.items():
        ...
        is_lawn = cid in s.lawn_constellation_ids_set
    ...
    for cid, cdata in s.constellation_data.items():
        if cdata.get("is_lawn"):
            s.lawn_system_ids.update(cdata["systems"].keys())
    ...
    tke_count = sum(len(c["systems"]) for c in s.constellation_data.values())
    lawn_count = len(s.lawn_system_ids)
