...
- [x] **Entosis command node board** — dedicated `/entosis` SPA page (`EntosisPage.jsx`); pilots enter a callsign, claim nodes, and advance status through unclaimed → running → contested → captured/lost; password-gated add/delete; 10s auto-poll; `routes/entosis_routes.py` + SQLite `entosis_nodes` table
