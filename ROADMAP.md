...
- [x] **Friendly entity expansion** — allied alliances added to the friendly lists; all 12 LAWN member corps added to `FRIENDLY_CORPORATIONS`
...
- [x] **Ally expansion** — further allied alliances added to the friendly lists; classified as FRIENDLY in local scanner and sov display
...
- [x] **Standalone-corp standings support** — new `FRIENDLY_STANDING_CORPORATIONS` deployment key for corps on the standings list that no alliance id covers; `FRIENDLY_STANDING_CORP_IDS`/`_NAMES` sets consumed by intel, local-scan, and hostile-feed routes. Added to support a bulk coalition import; the roster itself lives in the private deployment
...
- [x] **Traffic analytics** — answers "is this still worth running?": `routes/analytics_routes.py` registers an app-wide `after_app_request` hook that counts every request into two rollup tables (`traffic_hourly` keyed on hour/kind/path, `traffic_visitors` one row per visitor per day) — no per-request rows, so a busy day costs a few hundred rows regardless of poll volume. Counts buffer in memory and flush on an interval (and once per poller cycle) so SQLite writes stay off the request path. Privacy-preserving: no IPs or user agents stored, a visitor is a truncated HMAC of IP + user agent keyed on `ANALYTICS_SALT` (defaults to `FLASK_SECRET_KEY`); logged-in SSO sessions stamp the character name. Bot user agents counted separately and never as visitors; parameterised API paths collapse to their `url_rule`; unknown page paths bucket to `/other`. Reading the stats is operator-only and closed by default — `require_analytics_auth` takes an explicit character allowlist (`ANALYTICS_ALLOWED_CHARACTER_IDS`) or a dedicated `ANALYTICS_PASSWORD` (`X-Analytics-Auth` header), never the fleet-wide write credential, since `TIMER_PASSWORD` is handed out for timers and SSO write access covers the whole alliance; constant-time comparison plus a `RATELIMIT_ANALYTICS` cap (20/min) makes guessing impractical, and the header USAGE link only appears on a browser that has already unlocked the page. `GET /api/analytics/summary?days=N` (1–365) serves unique/returning visitors, "regulars" (5+ days), daily series, hour-of-day distribution, top pages/endpoints, and known pilots; rendered by the `/analytics` page (`AnalyticsPage.jsx`, linked as USAGE in the header). Rollups pruned after `ANALYTICS_RETENTION_DAYS` (default 180); recording disabled with `ANALYTICS_ENABLED=false`

- [x] **Shareable parser results** — the D-Scan, Local Scanner and Fleet Comp panels each publish their current result to a read-only page at `/s/<token>`, so intel can be passed on as a rendered page instead of the flattened plain text the COPY button produces. Backed by a `shared_reports` table and `routes/share_routes.py` (`POST /api/share` write-auth gated + rate limited, payload capped at 256 KB and **rejected** rather than truncated over it; `GET /api/share/<token>` public, 404 for both unknown and expired so it never confirms a token existed). Snapshots are frozen at publish time rather than re-derived on view — a local scan's standings and risk tiers are only true as of when they were taken, and replaying the lookups would both change the answer under the reader and spend the rate-limited zKill/ESI fan-out per page load; the page states its own age prominently for the same reason. Audience is per-share: `link` (anyone holding the URL) or `alliance` (viewer must pass the write-auth gate), switched by re-publishing so an already-pasted link is never repointed. Shares expire after `SHARE_RETENTION_DAYS` (default 3), pruned by the poller. `/s/<token>` collapses to `/s` in the analytics classifier so a token never lands in `traffic_hourly.path`, and `robots.txt` disallows `/s/`. Enabled by extracting each parser's result rendering into `frontend/src/components/parsers/<X>Result.jsx` (one renderer for the live panel and the shared page), exporting the copy-text builders to `utils/parserCopy.js` where they are now unit-tested, and collapsing the four duplicated clipboard buttons into `common/CopyButton.jsx`. `IntelChannelParser` is excluded on purpose — it is a rolling board, not a one-shot parse

- [x] **Reverse proxy trust** — `routes/proxy.py` + `TRUSTED_PROXY_HOPS` (default 0, set 1 behind `nginx.conf`) applies Werkzeug `ProxyFix` so `request.remote_addr` is the real client. Without it every user behind nginx shared a single per-IP rate-limit bucket, so one noisy visitor could throttle the intel endpoints for the whole alliance; trust is explicit config because `X-Forwarded-For` is client-forgeable, and a one-shot warning fires when forwarded requests arrive with the setting at 0. Analytics visitor hashing follows the same setting
---
